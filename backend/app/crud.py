...
def get_or_create_user(db: Session, username: str, display_name: str = None) -> models.User:
    user = db.query(models.User).filter_by(username=username).first()
    if user:
        return user
    user = models.User(
        username=username,
        display_name=(display_name or "Learner").strip()[:30] or "Learner",
...
def get_or_create_default_user(db: Session) -> models.User:
    """Fallback account used only when no X-User-Id header is sent
    (e.g. testing directly via /docs)."""
    return get_or_create_user(db, DEFAULT_USERNAME, "Alex")
