...
│   │   ├── deps.py           # Resolves the current user from the X-User-Id header
...
│   │       ├── leaderboard.py
│   │       └── session.py    # /api/session — creates a new learner account
│   └── requirements.txt
└── frontend/
    ├── app/
...
|---|---|---|
| POST | `/api/session` | `{display_name}` → creates a new learner account, returns its `username` (the frontend stores this as the caller's identity) |
| GET | `/api/me` | Current user (resolved from the `X-User-Id` header) + live stats (also passively regenerates hearts over time) |
...
- **Auth:** there's no password/login screen (as explicitly allowed by the
  assignment), but the app **does** support multiple distinct learners. On
  first visit, a person is asked for a display name; the backend creates a
  real `users` row for them and returns its `username`, which the frontend
  stores in `localStorage` and sends back as an `X-User-Id` header on every
  request (see `backend/app/deps.py` and `frontend/lib/session.ts`). Each
  browser/profile therefore gets its own XP, streak, hearts, and skill
  progress, and everyone shows up correctly on the shared leaderboard. This
  is intentionally *not* real authentication (no passwords, tokens, or
  server-side sessions) — it's the lightest mechanism that still satisfies
  "every learner's progress must persist" without requiring a login flow.
  Hitting the API directly (e.g. via `/docs`) with no header falls back to
  the seeded demo account (`learner1`). You can test with multiple learners
  yourself by opening the app in a normal window and an incognito window, or
  via Profile → **Switch learner**, which clears the stored id and re-prompts.
